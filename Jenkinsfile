...
pipeline {
    ...
    environment {
        ENV_TYPE = "production"
        PORT = 3850
        NAMESPACE = "socialpicturestore-ru"
        REGISTRY_HOSTNAME = "socialpicturestore"
        REGISTRY = "registry.hub.docker.com"
        PROJECT = "socialpicturestore"
        DEPLOYMENT_NAME = "socialpicturestore-deployment"
        ...
        stage('Push docker image') {
             steps {
                 echo "Push image started..."
                     script {
                          docker.withRegistry("https://${env.REGISTRY}", 'socialpicturestore-ru') {
